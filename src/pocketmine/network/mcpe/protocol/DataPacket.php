...
abstract class DataPacket extends NetworkBinaryStream{
	...
	public function decode(int $playerProtocol){
		$this->offset = 0;
		$this->decodeHeader();
		$this->decodePayload($playerProtocol);
	}
	...
	public function getEncodingProtocol(int $playerProtocol): int {
		$versions = $this->getProtocolVersions();

		if (in_array($playerProtocol, $versions, true)) {
			return $playerProtocol;
		}

		$closestProtocolId = ProtocolInfo::CURRENT_PROTOCOL;
		foreach ($versions as $version) {
			if ($version < $playerProtocol && (abs($playerProtocol - $closestProtocolId) > abs($version - $playerProtocol))) {
	...
	* @param int $playerProtocol
	 * @return void
	 */
	public function encode(int $playerProtocol = ProtocolInfo::CURRENT_PROTOCOL){
		$this->reset();
		$this->encodeHeader();
		$this->encodePayload($playerProtocol);
		$this->isEncoded = true;
	}
